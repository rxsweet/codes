                ...
                if res.is_redirect:
                    ...
                    if url == new_url:
                        ...
                        if no & REDIRECT_ORIGIN and no & REDIRECT_PATH_QUERY:
                            break
                        old, new = map(urlsplit, (url, new_url))
                        if (no & REDIRECT_ORIGIN and old[:2] != new[:2]) or (no & REDIRECT_PATH_QUERY and old[2:4] != new[2:4]):
                            break
                        url = new_url
...
class _ROSession(Session):
    ...
    def request(self, method, url='', *args, **kwargs):
        ...
        if self.__first and urlsplit(r.url).netloc != urlsplit(urljoin(self.base, url)).netloc:
