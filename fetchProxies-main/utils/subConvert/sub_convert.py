...
class sub_convert():
    ...
    def makeup(input, dup_rm_enabled=False, format_name_enabled=False): # 输入节点配置字典, 对节点进行区域的筛选和重命名，输出 YAML 文本 
        ...
        """
        #检测，删除错误端口节点：port中带'/?plugin='
        check = 0
        while check < len(proxies_list):
            if '/?plugin=' in str(proxies_list[check]):
                print('\n 端口出现/?plugin=错误，url = '+ str(proxies_list[check]) +'\n')
                proxies_list.pop(check)
                check  = check - 1
            check = check + 1
        """
            ...
            if 'ssr://' in line:
                try:
                    ...
                    if '^' in password_part:
                        ...
                        password_part = '12345678'#暂时这样处理，不知道为什么下面的pass不起作用
                        pass
                    yaml_url.setdefault('password', password_part)
                    
                    if '^' in password_part:#测试上面pass为什么没起作用，运行到这里了
                        print('\n password_part in pass = '+password_part+'\n')
