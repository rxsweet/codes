...
from utils import cached, list_file_paths, parallel_map
...
GH_RAW_URL_PREFIX = f'https://ghraw.netlify.app/{GITHUB_REPOSITORY}/{GITHUB_REF_NAME}'
GH_RAW_URL_PREFIX_SHA = f'https://ghraw.netlify.app/{GITHUB_REPOSITORY}/{GITHUB_SHA}'
GH_RAW_URL_PREFIX_2 = f'https://raw.kgithub.com/{GITHUB_REPOSITORY}/{GITHUB_REF_NAME}'
GH_RAW_URL_PREFIX_SHA_2 = f'https://raw.kgithub.com/{GITHUB_REPOSITORY}/{GITHUB_SHA}'
...
def access_ddal():
    if DDAL_EMAIL and DDAL_PASSWORD:
        try:
            return requests.head('https://dd.al').ok
        except Exception:
            pass
    return False
...
def get_ddal():
    if DDAL_EMAIL and DDAL_PASSWORD:
        try:
            ...
            print(f'短链服务 dd.al 登录失败：{e}')
    return None


def get_pp_url(path: str):
    if access_ddal():
        dirname, basename = os.path.split(path)
        if dirname == 'trials_providers':
            name = os.path.splitext(basename)[0]
            return f"https://dd.al/{get_alias(name)}"
    return f'{GH_RAW_URL_PREFIX_2}/{path}'
...
class DDAL:
    ...
    def login(self, email, password):
        ...
        for _ in range(20):
            ...
            raise Exception(f'尝试 20 次登录均失败 (loc = {repr(loc)})')
...
if __name__ == '__main__':
    def name_url_desc_iter(prefix1, prefix2):
        yield 'base64', f'{prefix1}/trial', 'base64 版'
        yield 'clash-hardcode', f'{prefix1}/trial.yaml', 'clash 硬编码版'
        yield 'clash-proxy-providers', f'{prefix1}/trial_pp.yaml', 'clash 提供器版'

        for path in list_file_paths('trials_providers'):
            name = os.path.splitext(os.path.basename(path))[0]
            yield name, f'{prefix2}/{path}', name

    if ddal := get_ddal():
        names, urls, descriptions = zip(*name_url_desc_iter(GH_RAW_URL_PREFIX_SHA, GH_RAW_URL_PREFIX_SHA_2))
        ...
        for short, description in zip(parallel_map(upsert, map(get_alias, names), urls), descriptions):
            print(f'{description}: {short}')
    else:
        for _, url, description in name_url_desc_iter(GH_RAW_URL_PREFIX, GH_RAW_URL_PREFIX_2):
