    ...
    if True: #开关：True，False
        ...
        while begin < length:
            ...
            if proxies_list[begin]['type'] == 'ss' or proxies_list[begin]['type'] == 'trojan' or proxies_list[begin]['type'] == 'ssr':  
                if isinstance(proxies_list[begin]['password'],int) or proxies_list[begin]['password'] == None or proxies_list[begin]['password'] == '':
                    #print(f"{proxies_list[begin]['name']}的password为int类型纯数字或空！")
                    #print(proxies_list[begin])
                    proxies_list.pop(begin)
                    length -= 1
                    begin -= 1
                elif proxies_list[begin]['password'].isdecimal():
                    #print(f"{proxies_list[begin]['name']}的password为纯数字的字符串")
                    proxies_list.pop(begin)
                    length -= 1
                    begin -= 1     
            begin += 1 
