        ...
        for line in lines:
            ...
            if 'ssr://' in line:
                ...
                try:
                    ...
                    ob = safe_base64_decode(param_dic['obfsparam'])
                    if '""' in ob:
                        print('yaml_encode 解析 ssr 节点出错：----- 字符串 "" 出现在ssr节点的obfsparam中')
                        continue
                    yaml_url.setdefault('obfsparam', ob)
                    proto = safe_base64_decode(param_dic['protoparam'])
                    if '""' in proto:
                        print('yaml_encode 解析 ssr 节点出错：----- 字符串 "" 出现在ssr节点的protoparam中')
                        continue
                    yaml_url.setdefault('protoparam', proto)
