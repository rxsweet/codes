...
class update():
            ...
            try:
                if current_remarks == 'pojiezhiyuanjun':
                    ...
                    if new_url == sub ['url']:
                        ...
                        sub['url'] = new_url
                        print(f'ID {current_remarks} url updated to {new_url}\n')
                elif current_remarks == 'freenode.me':
                    new_url = self.freenodeUpdate(sub ['url'])
                    if new_url == sub ['url']:
                        print(f'No available update for ID {current_remarks}\n')
                    else:
                        sub['url'] = new_url
    ...
    # ========== 抓取 freenode.me 的节点 ==========
    def freenodeUpdate(self,current_url):
        try:
            res = requests.get("https://freenode.me/f/freenode",headers=headers)
            #搜索可以借鉴下面这个
            article_url = re.search(r'<a href="(https://freenode.me/(.*?)\.html)" target="_self"></span>(.*?)最新免费节点(.*?)</a>',res.text).groups()[0]
            res = requests.get(article_url,headers=headers)
            new_url = re.search(r'<p>(https://freenode.me/wp-content/uploads/(.*?)\.yaml)</p>',res.text).groups()[0]
			#new_url = 'https://freenode.me/wp-content/uploads/' + new_url + '.yaml'
            #下面代码是获取订阅节点内容的，并且需要import base64
            #res = requests.get(sub_url,headers=headers)
            #merge += str(base64.b64decode(res.text.encode()),'utf-8').strip().replace('\r\n','\n').split('\n')
        except:
